...
function openPage(id){if(id!=="lesson"&&typeof stopSpeech==="function")stopSpeech({silent:true});const publicPages=new Set(["account","about","privacy","terms"]);if(typeof learningAccessGranted!=="undefined"&&!learningAccessGranted&&!publicPages.has(id)){if(typeof showAccessGate==="function")showAccessGate("Sign in with a registered email before opening FNP Quest.");return false;}document.querySelectorAll(".page").forEach(el=>el.classList.remove("active"));const page=document.getElementById(id);if(page)page.classList.add("active");window.scrollTo(0,0);if(typeof update==="function")update();return Boolean(page);}
